...
averages_by_rank = calc_averages_by_rank (M.link_sources) # returns dictionary
stdevs_by_rank   = calc_stdevs_by_rank (M.link_sources) # returns dictionary
source_zscores = {}
for i, link_source in enumerate(M.link_sources):
    rank   = get_rank_of_list (link_source)
    value  = M.link_sources[link_source]
    zscore = calc_zscore (value, averages_by_rank[rank], stdevs_by_rank[rank])
